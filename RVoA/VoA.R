## The Vortex of Accuracy, Version 4.1.5
...
## starting if loop for reading in data
if (as.numeric(week) <= 4) {
  ...
  OffUrl <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/total/season/2021"
  ...
  Off_Pass_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/passing"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  ...
  Off_Rush_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/rushing"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  ...
  DefUrl <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/total/table/passing/sort/yardsPerGame/dir/asc"
  ...
  ReturnUrl <- "https://www.espn.com/college-football/stats/team/_/view/special/stat/returning/table/returning/sort/yardsPerKickReturn/dir/desc"
  ...
  KickingUrl <- "https://www.espn.com/college-football/stats/team/_/view/special/stat/kicking/table/kicking/sort/fieldGoalPct/dir/desc"
  ...
  ## Downs stats
  DownsUrl <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/downs/table/miscellaneous/sort/firstDowns/dir/desc"
  ...
  ## Pass Defense stats
  PassDefUrl <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/passing/table/passing/sort/passingYardsPerGame/dir/asc"
  ...
  ## Rush Defense stats
  RushDefUrl <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/rushing/table/rushing/sort/rushingYardsPerGame/dir/asc"
  ...
  ## FPI resume stats
  FPIResumeURL <- "https://www.espn.com/college-football/fpi/_/view/resume"
  ...
  FPIEfficiencyURL <- "https://www.espn.com/college-football/fpi/_/view/efficiencies"
  ...
  ## Now scraping 2020 data, no tables merged yet
  ## scraping ESPN's website with Thomas Mock's snippet from rvest found on twitter
  
  ## Offensive Points and Yards Stats
  Off_2020_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/total/season/2020"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  Off_2020_StatsTeams <- Off_2020_Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(OffStatsTeams)
  Off_2020_StatsTable <- Off_2020_Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Off_2020_StatsTable[,1:4])
  ## merging team and stats tables
  Off_2020_StatsTable <- cbind(Off_2020_StatsTeams, Off_2020_StatsTable)
  ## head(Off_2020_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewOff_2020_ColNames <- c("Team", "Games_2020", "Total_Yds_2020", "Total_Yds_pg_2020",
                            "Pass_Yds_2020", "Pass_Yds_pg_2020", "Rush_Yds_2020", "Rush_Yds_pg_2020",
                            "Points_2020", "Pts_pg_2020")
  colnames(Off_2020_StatsTable) <- NewOff_2020_ColNames
  colnames(Off_2020_StatsTable)
  ## head(Off_2020_StatsTable[,1:4])
  Off_2020_StatsTable <- Off_2020_StatsTable[-c(1),]
  ## head(Off_2020_StatsTable[,1:4])
  
  ## some columns have commas
  # must get rid of these using gsub() function
  Off_2020_StatsTable$Total_Yds_2020 = gsub(",","", Off_2020_StatsTable$Total_Yds_2020)
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## keeping total yds stat for a Yards per play stat later on
  Off_2020_StatsTable <- Off_2020_StatsTable[,c("Team", "Games_2020", "Total_Yds_2020",
                                                "Total_Yds_pg_2020","Pass_Yds_pg_2020", 
                                                "Rush_Yds_pg_2020", "Points_2020", 
                                                "Pts_pg_2020")]
  
  
  ## Offensive Passing Stats
  Off_2020_Pass_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/passing/season/2020"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  Off_2020_Pass_Stats_Teams <- Off_2020_Pass_Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(Off_2020_Pass_Stats_Teams)
  Off_2020_Pass_Stats_Table <- Off_2020_Pass_Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Off_2020_Pass_Stats_Table[,1:4])
  ## merging team and stats tables
  Off_2020_Pass_Stats_Table <- cbind(Off_2020_Pass_Stats_Teams, Off_2020_Pass_Stats_Table)
  ## head(Off_2020_Pass_Stats_Table[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  ...
  colnames(Off_2020_Pass_Stats_Table)
  ## head(Off_2020_Pass_Stats_Table[,1:4])
  ## for some reason, total off and def stats pages include row containing column labels when scraped from ESPN's website
  # passing and rush pages do not do this, so no need to remove top column
  ## Off_2020_Pass_Stats_Table <- Off_2020_Pass_Stats_Table[-c(1),]
  ## head(Off_2020_Pass_Stats_Table[,1:4])
  ## some columns have commas
  # must get rid of these using gsub() function
  ...
  ## Offensive Rushing Stats
  Off_2020_Rush_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/rushing/season/2020"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  Off_2020_Rush_Stats_Teams <- Off_2020_Rush_Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(Off_2020_Rush_Stats_Teams)
  Off_2020_Rush_Stats_Table <- Off_2020_Rush_Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Off_2020_Rush_Stats_Table[,1:4])
  ## merging team and stats tables
  Off_2020_Rush_Stats_Table <- cbind(Off_2020_Rush_Stats_Teams, Off_2020_Rush_Stats_Table)
  ## head(Off_2020_Rush_Stats_Table[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  New_Off_2020_Rush_ColNames <- c("Team", "Games_2020", "Rush_Atts_2020", "Total_Rush_Yds_2020", 
                                  "Off_2020_Rush_YPA", "Off_2020_Rush_Yds_pg", "Off_Longest_Rush_2020", 
                                  "Rush_TDs_2020")
  colnames(Off_2020_Rush_Stats_Table) <- New_Off_2020_Rush_ColNames
  colnames(Off_2020_Rush_Stats_Table)
  ## head(Off_2020_Rush_Stats_Table[,1:4])
  ## for some reason, total off and def stats pages include row containing column labels when scraped from ESPN's website
  # passing and rush pages do not do this, so no need to remove top column
  ## Off_2020_Rush_Stats_Table <- Off_2020_Rush_Stats_Table[-c(1),]
  ## head(Off_2020_Rush_Stats_Table[,1:4])
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats, games already in Off_stats
  ## keeping attempts stat for yards per play stat later on
  Off_2020_Rush_Stats_Table <- Off_2020_Rush_Stats_Table[,c("Team", "Rush_Atts_2020","Off_2020_Rush_YPA")]
  
  
  
  ## Defensive Yards and Points Stats
  Def_2020_Url <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/total/season/2020"
  Def_2020_StatsTeams <- Def_2020_Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(Def_2020_StatsTeams)
  Def_2020_StatsTable <- Def_2020_Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Def_2020_StatsTable[,1:4])
  Def_2020_StatsTable <- cbind(Def_2020_StatsTeams, Def_2020_StatsTable)
  ## head(Def_2020_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewDef_2020_ColNames <- c("Team", "Games_2020", "Def_Total_Yds_2020", "Def_Total_Yds_pg_2020",
                            "Def_Pass_Yds_2020", "Def_Pass_Yds_pg_2020", "Def_Rush_Yds_2020", 
                            "Def_Rush_Yds_pg_2020", "Def_Pts_2020", "Def_Pts_pg_2020")
  colnames(Def_2020_StatsTable) <- NewDef_2020_ColNames
  colnames(Def_2020_StatsTable)
  ## head(Def_2020_StatsTable)
  Def_2020_StatsTable <- Def_2020_StatsTable[-c(1),]
  ## head(Def_2020_StatsTable[,1:4])
  
  ## some columns have commas
  # must get rid of these using gsub() function
  Def_2020_StatsTable$Def_Total_Yds_2020 = gsub(",","", Def_2020_StatsTable$Def_Total_Yds_2020)
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  ## keeping total yards stat for a yards per play stat later on
  Def_2020_StatsTable <- Def_2020_StatsTable[,c("Team", "Def_Total_Yds_2020",
                                                "Def_Total_Yds_pg_2020",
                                                "Def_Pass_Yds_pg_2020", 
                                                "Def_Rush_Yds_pg_2020", 
                                                "Def_Pts_2020", 
                                                "Def_Pts_pg_2020")]
  
  
  ## Special Teams kickoff and punt return stats
  Return_2020Url <- "https://www.espn.com/college-football/stats/team/_/view/special/stat/returning/season/2020"
  Return_2020StatsTeams <- Return_2020Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(Return_2020StatsTeams)
  Return_2020StatsTable <- Return_2020Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Return_2020StatsTable[,1:4])
  Return_2020StatsTable <- cbind(Return_2020StatsTeams, Return_2020StatsTable)
  ## head(Return_2020StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewReturn_2020ColNames <- c("Team", "Games_2020", "Kick_Return_Atts_2020", 
                              "Total_Kick_Return_Yds_2020","Kick_Return_Yd_Avg_2020", 
                              "Longest_Kick_Return_2020", "Kick_Return_TD_2020", 
                              "Punt_Return_Atts_2020", "Punt_Return_Yds_2020", 
                              "Punt_Return_Yd_Avg_2020", "Longest_Punt_Return_2020", 
                              "Punt_Return_TD_2020")
  colnames(Return_2020StatsTable) <- NewReturn_2020ColNames
  colnames(Return_2020StatsTable)
  ## head(Return_2020StatsTable[,1:4])
  Return_2020StatsTable <- Return_2020StatsTable[-c(1),]
  
  ## changing Return_2020StatsTable to have numeric values for non-team columns
  # need to do this early to normalized kick and punt Return_2020 columns
  Return_2020StatsTable[,2:ncol(Return_2020StatsTable)] <- sapply(Return_2020StatsTable[,2:ncol(Return_2020StatsTable)], as.numeric)
  ## changing fumbles forced column to a per-game stat to avoid biasing towards teams which played more game
  Return_2020StatsTable <- Return_2020StatsTable %>%
    dplyr::mutate(Kick_Return_TD_pg_2020 = Kick_Return_TD_2020/Games_2020) %>%
    dplyr::mutate(Punt_Return_TD_pg_2020 = Punt_Return_TD_2020/Games_2020)
  ## head(Return_2020StatsTable[,1:4])
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  Return_2020StatsTable <- Return_2020StatsTable[,c("Team", "Kick_Return_Yd_Avg_2020", 
                                                    "Kick_Return_TD_pg_2020", "Punt_Return_Yd_Avg_2020", 
                                                    "Punt_Return_TD_pg_2020")]
  
  ## Special Teams kicking stats
  Kicking_2020Url <- "https://www.espn.com/college-football/stats/team/_/view/special/stat/kicking/season/2020/table/kicking/sort/fieldGoalPct/dir/desc"
  Kicking_2020StatsTeams <- Kicking_2020Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(Kicking_2020StatsTeams)
  Kicking_2020StatsTable <- Kicking_2020Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Kicking_2020StatsTable[,1:4])
  Kicking_2020StatsTable <- cbind(Kicking_2020StatsTeams, Kicking_2020StatsTable)
  ## head(Kicking_2020StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewKicking_2020ColNames <- c("Team", "Games_2020", "FGM_2020", "FGA_2020","FG_Pct_2020", 
                               "Longest_FG_2020", "FG_1_19_Yds_2020", "FG_20_29_Yds_2020", 
                               "FG_30_39_Yds_2020", "FG_40_49_Yds_2020", "FG_50_Plus_Yds_2020", 
                               "Extra_Pts_Made_2020", "Extra_Pts_Attempted_2020", 
                               "Extra_Pts_Pct_2020")
  colnames(Kicking_2020StatsTable) <- NewKicking_2020ColNames
  colnames(Kicking_2020StatsTable)
  ## head(Kicking_2020StatsTable[,1:4])
  Kicking_2020StatsTable <- Kicking_2020StatsTable[-c(1),]
  ## head(Kicking_2020StatsTable[,1:4])
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  Kicking_2020StatsTable <- Kicking_2020StatsTable[,c("Team", "FG_Pct_2020",
                                                      "Extra_Pts_Pct_2020")]
  
  ## Downs stats
  Down_2020Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/downs/season/2020"
  Down_2020StatsTeams <- Down_2020Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(Down_2020StatsTeams)
  Down_2020StatsTable <- Down_2020Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(Down_2020StatsTable[,1:4])
  Down_2020StatsTable <- cbind(Down_2020StatsTeams, Down_2020StatsTable)
  ## head(Down_2020StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewDown_2020ColNames <- c("Team", "Games_2020", "Total_First_Downs_2020", "Rush_First_Downs_2020",
                            "Pass_First_Downs_2020", "Penalty_First_Downs_2020", "Third_Down_Made_2020", 
                            "Third_Down_Attempted_2020", "Third_Down_Pct_2020", 
                            "Fourth_Down_Made_2020", "Fourth_Down_Attempt_2020", "Fourth_Down_Pct_2020", 
                            "Penalties_2020", "Penalty_Yds_2020")
  colnames(Down_2020StatsTable) <- NewDown_2020ColNames
  colnames(Down_2020StatsTable)
  ## head(Down_2020StatsTable[,1:4])
  ## Top row is just original espn column names repeated
  Down_2020StatsTable <- Down_2020StatsTable[-c(1),]
  
  ## some columns have commas
  # removing commas
  Down_2020StatsTable$Penalty_Yds_2020 = gsub(",","", Down_2020StatsTable$Penalty_Yds_2020)
  
  ## changing Down_2020StatsTable to have numeric values for non-team columns
  # need to do this early to normalized first Down_2020 and penalty columns
  Down_2020StatsTable[,2:ncol(Down_2020StatsTable)] <- sapply(Down_2020StatsTable[,2:ncol(Down_2020StatsTable)], as.numeric)
  ## adding first downs and penalty stats as per-game stats to avoid biasing towards teams which played more games
  Down_2020StatsTable <- Down_2020StatsTable %>%
    dplyr::mutate(First_Downs_pg_2020 = Total_First_Downs_2020/Games_2020) %>%
    dplyr::mutate(Penalties_pg_2020 = Penalties_2020/Games_2020) %>%
    dplyr::mutate(Penalty_Yds_pg_2020 = Penalty_Yds_2020/Games_2020)
  ## head(Down_2020StatsTable[,1:4])
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  Down_2020StatsTable <- Down_2020StatsTable[,c("Team", "First_Downs_pg_2020",
                                                "Third_Down_Pct_2020",
                                                "Fourth_Down_Pct_2020", 
                                                "Penalties_pg_2020", "Penalty_Yds_pg_2020")]
  
  ## Pass Defense stats
  PassDef_2020_Url <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/passing/season/2020"
  PassDef_2020_StatsTeams <- PassDef_2020_Url %>% read_html() %>% html_table() %>% .[[1]]
  head(PassDef_2020_StatsTeams)
  PassDef_2020_StatsTable <- PassDef_2020_Url %>% read_html() %>% html_table() %>% .[[2]]
  head(PassDef_2020_StatsTable[,1:4])
  PassDef_2020_StatsTable <- cbind(PassDef_2020_StatsTeams, PassDef_2020_StatsTable)
  head(PassDef_2020_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewPassDef_2020_ColNames <- c("Team", "Games_2020", "Completions_Allowed_2020", 
                                "Pass_Atts_Allowed_2020","Def_Comp_Pct_Allowed_2020", 
                                "Passing_Yds_Allowed_2020", "Def_Pass_YPA_2020", 
                                "Passing_Yds_Allowed_pg_2020", "Longest_Pass_Allowed_2020", 
                                "Passing_Tds_Allowed_2020", "Interceptions_2020", "Def_Sacks_2020", 
                                "Def_Sack_Yds_Lost_2020", "Passer_Rtg_Allowed_2020")
  colnames(PassDef_2020_StatsTable) <- NewPassDef_2020_ColNames
  colnames(PassDef_2020_StatsTable)
  ## head(PassDef_2020_StatsTable[,1:4])
  
  ## changing PassDef_2020_StatsTable to have numeric values for non-team columns
  # need to do this early to add interceptions_pg column
  PassDef_2020_StatsTable[,2:ncol(PassDef_2020_StatsTable)] <- sapply(PassDef_2020_StatsTable[,2:ncol(PassDef_2020_StatsTable)], as.numeric)
  
  ## changing fumbles forced column to a per-game stat to avoid biasing towards teams which played more game
  PassDef_2020_StatsTable <- PassDef_2020_StatsTable %>%
    dplyr::mutate(Interceptions_pg_2020 = Interceptions_2020/Games_2020) %>%
    dplyr::mutate(Def_Sacks_pg_2020 = Def_Sacks_2020/Games_2020) %>%
    dplyr::mutate(Def_Sack_Yds_Lost_pg_2020 = Def_Sack_Yds_Lost_2020/Games_2020)
  
  ## Some of the ESPN tables scraped with top column being a repeat of column names
  ## For some reason, pass defense and rush defense do not do this
  ## PassDef_2020_StatsTable <- PassDef_2020_StatsTable[-c(1),]
  head(PassDef_2020_StatsTable[,1:4])
  ...
  PassDef_2020_StatsTable <- PassDef_2020_StatsTable[,c("Team", "Pass_Atts_Allowed_2020","Def_Comp_Pct_Allowed_2020", 
                                                        "Def_Pass_YPA_2020", 
                                                        "Interceptions_pg_2020", "Def_Sacks_pg_2020", 
                                                        "Def_Sack_Yds_Lost_pg_2020")]
  
  
  ## Rush Defense stats
  RushDef_2020_Url <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/rushing/season/2020"
  RushDef_2020_StatsTeams <- RushDef_2020_Url %>% read_html() %>% html_table() %>% .[[1]]
  ## head(RushDef_2020_StatsTeams)
  RushDef_2020_StatsTable <- RushDef_2020_Url %>% read_html() %>% html_table() %>% .[[2]]
  ## head(RushDef_2020_StatsTable[,1:4])
  RushDef_2020_StatsTable <- cbind(RushDef_2020_StatsTeams, RushDef_2020_StatsTable)
  ## head(RushDef_2020_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewRushDef_2020_ColNames <- c("Team", "Games_2020", "Rush_Att_Allowed_2020", 
                                "Rushing_Yards_Allowed_2020","Yds_Per_Rush_Attempt_Allowed_2020", 
                                "Rushing_Yards_pg_Allowed_2020", 
                                "Longest_Rush_Allowed_2020", "Rush_Tds_Allowed_2020", 
                                "Fumbles_Forced_2020")
  colnames(RushDef_2020_StatsTable) <- NewRushDef_2020_ColNames
  colnames(RushDef_2020_StatsTable)
  ## head(RushDef_2020_StatsTable[,1:4])
  ## changing RushDef_2020_StatsTable to have numeric values for non-team columns
  # need to do this early to add fumbles_forced_pg column
  RushDef_2020_StatsTable[,2:ncol(RushDef_2020_StatsTable)] <- sapply(RushDef_2020_StatsTable[,2:ncol(RushDef_2020_StatsTable)], as.numeric)
  
  ## changing fumbles forced column to a per-game stat to avoid biasing towards teams which played more game
  RushDef_2020_StatsTable <- RushDef_2020_StatsTable %>%
    dplyr::mutate(Fumbles_Forced_pg_2020 = Fumbles_Forced_2020/Games_2020)
  
  ## Some of the ESPN tables scraped with top column being a repeat of column names
  ## For some reason, pass defense and rush defense do not do this
  ## RushDef_2020_StatsTable <- RushDef_2020_StatsTable[-c(1),]
  ## head(RushDef_2020_StatsTable[,1:4])
  ...
  ## Rush yds per game already included in previous defensive stats table
  ## keeping attempts stat in for yards per play stat later on
  RushDef_2020_StatsTable <- RushDef_2020_StatsTable[,c("Team", "Rush_Att_Allowed_2020","Yds_Per_Rush_Attempt_Allowed_2020",
                                                        "Fumbles_Forced_pg_2020")]
  
  ## FPI resume stats
  FPI_2020_ResumeURL <- "https://www.espn.com/college-football/fpi/_/view/resume/season/2020"
  FPI_2020_Teams <- FPI_2020_ResumeURL %>% read_html() %>% html_table() %>% .[[1]]
  ## head(FPI_2020_Teams)
  FPI_2020_ResumeStats <- FPI_2020_ResumeURL %>% read_html() %>% html_table() %>% .[[2]]
  ## head(FPI_2020_ResumeStats[,1:4])
  FPI_2020_ResumeTable <- cbind(FPI_2020_Teams, FPI_2020_ResumeStats)
  ## head(FPI_2020_ResumeTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  FPI_2020_ResumeColNames <- c("Team", "FPI_Rank_2020", "AP_CFP_2020", "SOR_2020", "SOS_2020","Rem_SOS_2020", 
                               "Game_Control_Rank_2020", "Avg_WinProb_2020")
  colnames(FPI_2020_ResumeTable) <- FPI_2020_ResumeColNames
  colnames(FPI_2020_ResumeTable)
  head(FPI_2020_ResumeTable[,1:4])
  ## Some of the ESPN tables scraped with top column being a repeat of column names
  FPI_2020_ResumeTable <- FPI_2020_ResumeTable[-c(1),]
  head(FPI_2020_ResumeTable[,1:4])
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  FPI_2020_ResumeTable <- FPI_2020_ResumeTable[,c("Team", "FPI_Rank_2020","SOR_2020", "SOS_2020", 
                                                  "Game_Control_Rank_2020", "Avg_WinProb_2020")]
  
  FPI_2020_ResumeTable <- subset(FPI_2020_ResumeTable, SOS_2020 > 0)
  
  ## FPI efficiency stats
  FPI_2020_EfficiencyURL <- "https://www.espn.com/college-football/fpi/_/view/efficiencies/season/2020"
  FPI_2020_Teams2 <- FPI_2020_EfficiencyURL %>% read_html() %>% html_table() %>% .[[1]]
  ## head(FPI_2020_Teams2)
  FPI_2020_Efficiency <- FPI_2020_EfficiencyURL %>% read_html() %>% html_table() %>% .[[2]]
  ## head(FPI_2020_Efficiency[,1:4])
  FPI_2020_Efficiency <- cbind(FPI_2020_Teams2, FPI_2020_Efficiency)
  ## head(FPI_2020_Efficiency[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  FPI_2020_EfficiencyColNames <- c("Team", "FPI_Ovrl_Eff_2020", "FPI_Ovrl_Rk_2020", "FPI_Off_Eff_2020", 
                                   "FPI_Off_Rk_2020","FPI_Def_Eff_2020", "FPI_Def_Rk_2020", 
                                   "FPI_ST_Eff_2020", "FPI_ST_Rk_2020")
  colnames(FPI_2020_Efficiency) <- FPI_2020_EfficiencyColNames
  colnames(FPI_2020_Efficiency)
  ## head(FPI_2020_Efficiency[,1:4])
  ## Some of the ESPN tables scraped with top column being a repeat of column names
  FPI_2020_Efficiency <- FPI_2020_Efficiency[-c(1),]
  head(FPI_2020_Efficiency[,1:4])
  
  ## Selecting only variables which will be used for VoA
  FPI_2020_Efficiency <- FPI_2020_Efficiency[,c("Team", "FPI_Ovrl_Rk_2020", "FPI_Off_Rk_2020",
                                                "FPI_Def_Rk_2020", "FPI_ST_Rk_2020")]
  FPI_2020_Efficiency <- subset(FPI_2020_Efficiency, FPI_Ovrl_Rk_2020 > 0)
  ...
  # FPI columns will serve as first column in range of columns to be included
  # in rowmeans() function later on
  FPI_2020_ResumeTable[,2:ncol(FPI_2020_ResumeTable)] <- sapply(FPI_2020_ResumeTable[,2:ncol(FPI_2020_ResumeTable)], as.numeric)
  FPI_2020_Efficiency[,2:ncol(FPI_2020_Efficiency)] <- sapply(FPI_2020_Efficiency[,2:ncol(FPI_2020_Efficiency)], as.numeric)
  
  ## Adding means of FPI Efficiency stats (excluding overall)
  # first just offense and defense, 2nd includes special teams
  FPI_2020_Efficiency <- FPI_2020_Efficiency %>%
    mutate(FPI_Off_Def_Mean_Rk_2020 = rowMeans(FPI_2020_Efficiency[,2:3])) %>%
    mutate(FPI_Off_Def_ST_Mean_Rk_2020 = rowMeans(FPI_2020_Efficiency[,2:4]))
  
  
  ## Now scraping 2019 data, no tables merged yet
  ## need 2019 data for UConn, Old Dominion, and New Mexico St ONLY
  ## COLUMN LABELS INTENTIONALLY WRONGLY INCLUDE 2020 AS YEAR
  # this is done to make merging 2020 opt-out data from 2019 with 2020 table easier
  ## scraping ESPN's website with Thomas Mock's snippet from rvest found on twitter
  
  ## Offensive Points and Yards Stats
  Off_2019Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/total/season/2019"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  ...
  ## head(Off_2019_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewOff_2019_ColNames <- c("Team", "Games_2020", "Total_Yds_2020", "Total_Yds_pg_2020",
                            "Pass_Yds_2020", "Pass_Yds_pg_2020", "Rush_Yds_2020", "Rush_Yds_pg_2020",
                            "Points_2020", "Pts_pg_2020")
  ...
  ## some columns have commas
  # must get rid of these using gsub() function
  Off_2019_StatsTable$Total_Yds_2020 = gsub(",","", Off_2019_StatsTable$Total_Yds_2020)
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## keeping total yds stat for yards per play later on
  Off_2019_StatsTable <- Off_2019_StatsTable[,c("Team", "Games_2020", "Total_Yds_2020","Total_Yds_pg_2020",
                                                "Pass_Yds_pg_2020", "Rush_Yds_pg_2020",
                                                "Points_2020", "Pts_pg_2020")]
  
  
  ## Offensive Passing Stats
  Off_2019_Pass_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/passing/season/2019"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  ...
  ## Offensive Rushing Stats
  Off_2019_Rush_Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/rushing/season/2019"
  ## Team comes in as its own table for some reason, scraping team and stats separately
  ...
  ## head(Off_2019_Rush_Stats_Table[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  New_Off_2019_Rush_ColNames <- c("Team", "Games_2020", "Rush_Atts_2020", "Total_Rush_Yds_2020", 
                                  "Off_2020_Rush_YPA", "Off_2020_Rush_Yds_pg", "Off_Longest_Rush_2020", 
                                  "Rush_TDs_2020")
  ...
  ## eliminating sum stats, only keeping per-game stats, games already in Off_stats
  ## keeping attempts stat for yards per play stat later on
  Off_2019_Rush_Stats_Table <- Off_2019_Rush_Stats_Table[,c("Team", "Rush_Atts_2020", "Off_2020_Rush_YPA")]
  
  
  
  ## Defensive Yards and Points Stats
  Def_2019_Url <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/total/season/2019"
  ...
  ## head(Def_2019_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewDef_2019_ColNames <- c("Team", "Games_2020", "Def_Total_Yds_2020", "Def_Total_Yds_pg_2020",
                            "Def_Pass_Yds_2020", "Def_Pass_Yds_pg_2020", "Def_Rush_Yds_2020", 
                            "Def_Rush_Yds_pg_2020", "Def_Pts_2020", "Def_Pts_pg_2020")
  ...
  ## some columns have commas
  # must get rid of these using gsub() function
  Def_2019_StatsTable$Def_Total_Yds_2020 = gsub(",","", Def_2019_StatsTable$Def_Total_Yds_2020)
  
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  ## keeping total yds stat for yards per play stat later on
  Def_2019_StatsTable <- Def_2019_StatsTable[,c("Team", "Def_Total_Yds_2020", "Def_Total_Yds_pg_2020",
                                                "Def_Pass_Yds_pg_2020", 
                                                "Def_Rush_Yds_pg_2020", 
                                                "Def_Pts_2020", "Def_Pts_pg_2020")]
  
  
  ## Special Teams kickoff and punt return stats
  Return_2019Url <- "https://www.espn.com/college-football/stats/team/_/view/special/stat/returning/season/2019"
  ...
  ## head(Return_2019StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewReturn_2019ColNames <- c("Team", "Games_2020", "Kick_Return_Atts_2020", 
                              "Total_Kick_Return_Yds_2020","Kick_Return_Yd_Avg_2020", 
                              "Longest_Kick_Return_2020", "Kick_Return_TD_2020", 
                              "Punt_Return_Atts_2020", "Punt_Return_Yds_2020", 
                              "Punt_Return_Yd_Avg_2020", "Longest_Punt_Return_2020", 
  ...
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  Return_2019StatsTable <- Return_2019StatsTable[,c("Team", "Kick_Return_Yd_Avg_2020", 
                                                    "Kick_Return_TD_pg_2020", "Punt_Return_Yd_Avg_2020", 
                                                    "Punt_Return_TD_pg_2020")]
  
  ## Special Teams kicking stats
  Kicking_2019Url <- "https://www.espn.com/college-football/stats/team/_/view/special/stat/kicking/season/2019/table/kicking/sort/fieldGoalPct/dir/desc"
  ...
  ## head(Kicking_2019StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewKicking_2019ColNames <- c("Team", "Games_2020", "FGM_2020", "FGA_2020","FG_Pct_2020", 
                               "Longest_FG_2020", "FG_1_19_Yds_2020", "FG_20_29_Yds_2020", 
                               "FG_30_39_Yds_2020", "FG_40_49_Yds_2020", "FG_50_Plus_Yds_2020", 
                               "Extra_Pts_Made_2020", "Extra_Pts_Attempted_2020", 
                               "Extra_Pts_Pct_2020")
  ...
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  Kicking_2019StatsTable <- Kicking_2019StatsTable[,c("Team", "FG_Pct_2020",
                                                      "Extra_Pts_Pct_2020")]
  
  ## Downs stats
  Down_2019Url <- "https://www.espn.com/college-football/stats/team/_/view/offense/stat/downs/season/2019"
  ...
  ## head(Down_2019StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewDown_2019ColNames <- c("Team", "Games_2020", "Total_First_Downs_2020", "Rush_First_Downs_2020",
                            "Pass_First_Downs_2020", "Penalty_First_Downs_2020", "Third_Down_Made_2020", 
                            "Third_Down_Attempted_2020", "Third_Down_Pct_2020", 
                            "Fourth_Down_Made_2020", "Fourth_Down_Attempt_2020", "Fourth_Down_Pct_2020", 
                            "Penalties_2020", "Penalty_Yds_2020")
  ...
  ## eliminating sum stats, only keeping per-game stats
  ## kept games played in offensive stats table, don't need to leave it for other tables
  Down_2019StatsTable <- Down_2019StatsTable[,c("Team", "First_Downs_pg_2020",
                                                "Third_Down_Pct_2020",
                                                "Fourth_Down_Pct_2020", 
                                                "Penalties_pg_2020", "Penalty_Yds_pg_2020")]
  
  ## Pass Defense stats
  PassDef_2019_Url <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/passing/season/2019"
  ...
  head(PassDef_2019_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewPassDef_2019_ColNames <- c("Team", "Games_2020", "Completions_Allowed_2020", 
                                "Pass_Atts_Allowed_2020","Def_Comp_Pct_Allowed_2020", 
                                "Passing_Yds_Allowed_2020", "Def_Pass_YPA_2020", 
                                "Passing_Yds_Allowed_pg_2020", "Longest_Pass_Allowed_2020", 
                                "Passing_Tds_Allowed_2020", "Interceptions_2020", "Def_Sacks_2020", 
  ...
  ## pass yds per game already included in previous defensive stats table
  ## keeping attempts stat for yards per play stat later on
  PassDef_2019_StatsTable <- PassDef_2019_StatsTable[,c("Team", "Pass_Atts_Allowed_2020", "Def_Comp_Pct_Allowed_2020", 
                                                        "Def_Pass_YPA_2020", 
                                                        "Interceptions_pg_2020", "Def_Sacks_pg_2020", 
                                                        "Def_Sack_Yds_Lost_pg_2020")]
  ...
  ## Rush Defense stats
  RushDef_2019_Url <- "https://www.espn.com/college-football/stats/team/_/view/defense/stat/rushing/season/2019"
  ...
  ## head(RushDef_2019_StatsTable[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  NewRushDef_2019_ColNames <- c("Team", "Games_2020", "Rush_Att_Allowed_2020", 
                                "Rushing_Yards_Allowed_2020","Yds_Per_Rush_Attempt_Allowed_2020", 
                                "Rushing_Yards_pg_Allowed_2020", 
                                "Longest_Rush_Allowed_2020", "Rush_Tds_Allowed_2020", 
                                "Fumbles_Forced_2020")
  ...
  ## Rush yds per game already included in previous defensive stats table
  ## keeping attempts stat for yards per play stat later on
  RushDef_2019_StatsTable <- RushDef_2019_StatsTable[,c("Team", "Rush_Att_Allowed_2020", "Yds_Per_Rush_Attempt_Allowed_2020",
                                                        "Fumbles_Forced_pg_2020")]
  
  ## FPI resume stats
  FPI_2019_ResumeURL <- "https://www.espn.com/college-football/fpi/_/view/resume/season/2019"
  ...
  ## column names are off, appear as first row instead, renaming column names
  FPI_2019_ResumeColNames <- c("Team", "FPI_Rank_2020", "AP_CFP_2020", "SOR_2020", "SOS_2020","Rem_SOS_2020", 
                               "Game_Control_Rank_2020", "Avg_WinProb_2020")
  ...
  ## Selecting only variables which will be used for VoA
  ## eliminating sum stats, only keeping per-game stats
  FPI_2019_ResumeTable <- FPI_2019_ResumeTable[,c("Team", "FPI_Rank_2020","SOR_2020", "SOS_2020", 
                                                  "Game_Control_Rank_2020", "Avg_WinProb_2020")]
  
  FPI_2019_ResumeTable <- subset(FPI_2019_ResumeTable, SOS_2020 > 0)
  
  ## FPI efficiency stats
  FPI_2019_EfficiencyURL <- "https://www.espn.com/college-football/fpi/_/view/efficiencies/season/2019"
  ...
  ## head(FPI_2019_Efficiency[,1:4])
  ## column names are off, appear as first row instead, renaming column names
  ## column names will be given year of 2020 to match 2020 table so 2020 opt-outs
  # can be properly merged
  FPI_2019_EfficiencyColNames <- c("Team", "FPI_Ovrl_Eff_2020", "FPI_Ovrl_Rk_2020", "FPI_Off_Eff_2020", 
                                   "FPI_Off_Rk_2020","FPI_Def_Eff_2020", "FPI_Def_Rk_2020", 
                                   "FPI_ST_Eff_2020", "FPI_ST_Rk_2020")
  ...
  ## Selecting only variables which will be used for VoA
  FPI_2019_Efficiency <- FPI_2019_Efficiency[,c("Team", "FPI_Ovrl_Rk_2020", "FPI_Off_Rk_2020",
                                                "FPI_Def_Rk_2020", "FPI_ST_Rk_2020")]
  FPI_2019_Efficiency <- subset(FPI_2019_Efficiency, FPI_Ovrl_Rk_2020 > 0)
  ...
  FPI_2019_Efficiency <- FPI_2019_Efficiency %>%
    mutate(FPI_Off_Def_Mean_Rk_2020 = rowMeans(FPI_2019_Efficiency[,2:3])) %>%
    mutate(FPI_Off_Def_ST_Mean_Rk_2020 = rowMeans(FPI_2019_Efficiency[,2:4]))
} else {
  ## scraping ESPN's website with Thomas Mock's snippet from rvest found on twitter
...
# then previous year
## because of 2020 opt outs, 2020 will be merged by itself, as will 2019 data
# 2020 will be merged with 2019 data first, then it will all be merged together
...
if (as.numeric(week) <= 4) {
  ...
  ## now merging 2020 data
  Temp_2020_df <- merge(Off_2020_StatsTable, Off_2020_Pass_Stats_Table, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, Off_2020_Rush_Stats_Table, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, Def_2020_StatsTable, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, PassDef_2020_StatsTable, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, RushDef_2020_StatsTable, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, Down_2020StatsTable, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, Return_2020StatsTable, by = "Team")
  Temp_2020_df <- merge(Temp_2020_df, Kicking_2020StatsTable, by = "Team")
  ...
  ## subsetting 2019 data to only include 2020 opt-outs
  Temp_2019_df <- subset(Temp_2019_df, Team == "UConn Huskies" | Team == "New Mexico State Aggies" | Team == "Old Dominion Monarchs")
  ## Combining 2020 and subsetted 2019 data
  Temp_Prev_Year_df <- rbind(Temp_2020_df, Temp_2019_df)
  ## Now merging 2020 FPI data, needs to be done early to be joined with merged 2019 FPI data
  Temp_FPI_2020_df <- merge(FPI_2020_ResumeTable, FPI_2020_Efficiency, by = "Team")
  ...
  VoA_Variables <- VoA_Variables %>%
    ...
    mutate(YPP_2020 = Total_Yds_2020 / (Rush_Atts_2020 + Attempts_2020)) %>%
    mutate(Pts_per_play_2020 = Pts_pg_2020 / (Rush_Atts_2020 + Attempts_2020)) %>%
  ...
  SOS_Adj_MOV_2020_Vector <- VoA_Variables$MOV_2020 / FPI_2020_ResumeTable$SOS_2020
  SOR_Adj_MOV_Vector <- VoA_Variables$MOV / FPIResumeTable$SOR
  SOR_Adj_MOV_2020_Vector <- VoA_Variables$MOV / FPI_2020_ResumeTable$SOR_2020
  VoA_Variables <- VoA_Variables %>%
    mutate(SOS_Adj_MOV = SOS_Adj_MOV_Vector) %>%
    mutate(SOS_Adj_MOV_2020 = SOS_Adj_MOV_2020_Vector) %>%
    mutate(SOR_Adj_MOV = SOR_Adj_MOV_Vector) %>%
    mutate(SOR_Adj_MOV_2020 = SOR_Adj_MOV_2020_Vector)
...
if (as.numeric(week) <= 4) {
  VoA_Variables <- VoA_Variables[,c("Team", "Games", "Total_Yds_pg", "Pass_Yds_pg", 
                                    ...
                                    "Extra_Pts_Percent", "Games_2020",
                                    "Total_Yds_pg_2020", "Pass_Yds_pg_2020", 
                                    "Rush_Yds_pg_2020", "Pts_pg_2020", 
                                    "Comp_Pct_2020", "Pass_YPA_2020",
                                    "Off_YPC_2020","Off_INTs_pg_2020",
                                    "Off_Sacks_Allowed_pg_2020", 
                                    "Off_Sack_Yds_Lost_pg_2020",
                                    "Off_2020_Rush_YPA", "Def_Total_Yds_pg_2020",            
                                    "Def_Pass_Yds_pg_2020","Def_Rush_Yds_pg_2020",             
                                    "Def_Pts_pg_2020", "Def_Comp_Pct_Allowed_2020",
                                    "Def_Pass_YPA_2020", "Interceptions_pg_2020",
                                    "Def_Sacks_pg_2020", "Def_Sack_Yds_Lost_pg_2020",            
                                    "Yds_Per_Rush_Attempt_Allowed_2020",
                                    "Fumbles_Forced_pg_2020",   
                                    "First_Downs_pg_2020", "Third_Down_Pct_2020",
                                    "Fourth_Down_Pct_2020", "Penalties_pg_2020",
                                    "Penalty_Yds_pg_2020", "Kick_Return_Yd_Avg_2020",        
                                    ...
                                    "FPI_Rank_2020","SOR_2020","SOS_2020",
                                    "Game_Control_Rank_2020","Avg_WinProb_2020",
                                    "FPI_Ovrl_Rk_2020", "FPI_Off_Rk_2020",
                                    "FPI_Def_Rk_2020","FPI_ST_Rk_2020", 
                                    "FPI_Off_Def_Mean_Rk_2020",         
                                    "FPI_Off_Def_ST_Mean_Rk_2020")]
} else {
  VoA_Variables <- VoA_Variables[,c("Team", "Games", "Total_Yds_pg", "Pass_Yds_pg", 
                                    "Rush_Yds_pg", "Pts_pg", "Comp_Pct",
                                    "Pass_YPA", "Off_YPC", "Off_INTs_pg",           
...
if (as.numeric(week) <= 3) {
  VoA_Variables <- VoA_Variables %>%
    ...
    mutate(Rank_Yds_pg_2020 = dense_rank(desc(Total_Yds_pg_2020))) %>%
    mutate(Rank_Yds_pg_2020_Col2 = dense_rank(desc(Total_Yds_pg_2020))) %>%
    mutate(Rank_Pass_Yds_pg_2020 = dense_rank(desc(Pass_Yds_pg_2020))) %>%
    mutate(Rank_Pass_Yds_pg_2020_Col2 = dense_rank(desc(Pass_Yds_pg_2020))) %>%
    mutate(Rank_Rush_Yds_pg_2020 = dense_rank(desc(Rush_Yds_pg_2020))) %>%
    mutate(Rank_Rush_Yds_pg_2020_Col2 = dense_rank(desc(Rush_Yds_pg_2020))) %>%
    mutate(Rank_Pts_pg_2020 = dense_rank(desc(Pts_pg_2020))) %>%
    mutate(Rank_Pts_pg_2020_Col2 = dense_rank(desc(Pts_pg_2020))) %>%
    mutate(Rank_Comp_Pct_2020 = dense_rank(desc(Comp_Pct_2020))) %>%
    mutate(Rank_Pass_YPA_2020 = dense_rank(desc(Pass_YPA_2020))) %>%
    mutate(Rank_Pass_YPA_2020_Col2 = dense_rank(desc(Pass_YPA_2020))) %>%
    mutate(Rank_Off_YPC_2020 = dense_rank(desc(Off_YPC_2020))) %>%
    mutate(Rank_Off_INTs_pg_2020 = dense_rank(Off_INTs_pg_2020)) %>%
    mutate(Rank_Off_INTs_pg_2020_Col2 = dense_rank(Off_INTs_pg_2020)) %>%
    mutate(Rank_Off_Sacks_Allowed_pg_2020 = dense_rank(Off_Sacks_Allowed_pg_2020)) %>%
    mutate(Rank_Off_Sack_Yds_Lost_pg_2020 = dense_rank(Off_Sack_Yds_Lost_pg_2020)) %>%
    mutate(Rank_Off_Rush_YPA_2020 = dense_rank(desc(Off_2020_Rush_YPA))) %>%
    mutate(Rank_Def_Yds_pg_2020 = dense_rank(Def_Total_Yds_pg_2020)) %>%
    mutate(Rank_Def_Yds_pg_2020_Col2 = dense_rank(Def_Total_Yds_pg_2020)) %>%
    mutate(Rank_Def_Pass_Yds_Game_2020 = dense_rank(Def_Pass_Yds_pg_2020)) %>%
    mutate(Rank_Def_Pass_Yds_Game_2020_Col2 = dense_rank(Def_Pass_Yds_pg_2020)) %>%
    mutate(Rank_Def_Rush_Yds_Game_2020 = dense_rank(Def_Rush_Yds_pg_2020)) %>%
    mutate(Rank_Def_Rush_Yds_Game_2020_Col2 = dense_rank(Def_Rush_Yds_pg_2020)) %>%
    mutate(Rank_Def_Pts_Game_2020 = dense_rank(Def_Pts_pg_2020)) %>%
    mutate(Rank_Def_Pts_Game_2020_Col2 = dense_rank(Def_Pts_pg_2020)) %>%
    mutate(Rank_Kick_Return_Yd_Avg_2020 = dense_rank(desc(Kick_Return_Yd_Avg_2020))) %>%
    mutate(Rank_Kick_Return_TD_2020 = dense_rank(desc(Kick_Return_TD_pg_2020))) %>%
    mutate(Rank_Punt_Return_Yd_Avg_2020 = dense_rank(desc(Punt_Return_Yd_Avg_2020))) %>%
    mutate(Rank_Punt_Return_TD_2020 = dense_rank(desc(Punt_Return_TD_pg_2020))) %>%
    mutate(Rank_FG_Pct_2020 = dense_rank(desc(FG_Pct_2020))) %>%
    mutate(Rank_XP_Pct_2020 = dense_rank(desc(Extra_Pts_Pct_2020))) %>%
    mutate(Rank_1st_Downs_2020 = dense_rank(desc(First_Downs_pg_2020))) %>%
    mutate(Rank_3rd_Down_Pct_2020 = dense_rank(desc(Third_Down_Pct_2020))) %>%
    mutate(Rank_3rd_Down_Pct_2020_Col2 = dense_rank(desc(Third_Down_Pct_2020))) %>%
    mutate(Rank_4th_Down_Pct_2020 = dense_rank(desc(Fourth_Down_Pct_2020))) %>%
    mutate(Rank_Penalties_2020 = dense_rank(Penalties_pg_2020)) %>%
    mutate(Rank_Penalty_Yds_2020 = dense_rank(Penalty_Yds_pg_2020)) %>%
    mutate(Rank_Def_Comp_Pct_Allowed_2020 = dense_rank(Def_Comp_Pct_Allowed_2020)) %>%
    mutate(Rank_Def_Comp_Pct_Allowed_2020_Col2 = dense_rank(Def_Comp_Pct_Allowed_2020)) %>%
    mutate(Rank_Def_Pass_YPA_2020 = dense_rank(Def_Pass_YPA_2020)) %>%
    mutate(Rank_Def_Pass_YPA_2020_Col2 = dense_rank(Def_Pass_YPA_2020)) %>%
    mutate(Rank_Ints_2020 = dense_rank(desc(Interceptions_pg_2020))) %>%
    mutate(Rank_Ints_2020_Col2 = dense_rank(desc(Interceptions_pg_2020))) %>%
    mutate(Rank_Def_Sacks_pg_2020 = dense_rank(desc(Def_Sacks_pg_2020))) %>%
    mutate(Rank_Def_Sack_Yds_pg_2020 = dense_rank(desc(Def_Sack_Yds_Lost_pg_2020))) %>%
    mutate(Rank_Yds_Rush_Att_Allowed_2020 = dense_rank(Yds_Per_Rush_Attempt_Allowed_2020)) %>%
    mutate(Rank_Yds_Rush_Att_Allowed_2020_Col2 = dense_rank(Yds_Per_Rush_Attempt_Allowed_2020)) %>%
    mutate(Rank_Fumbles_Forced_2020 = dense_rank(desc(Fumbles_Forced_pg_2020))) %>%
    ...
    mutate(Rank_Def_YPP_2020_Col2 = Rank_Def_YPP_2020) %>%
    mutate(Rank_Pts_per_play_2020 = dense_rank(desc(Pts_per_play_2020))) %>%
    mutate(Rank_Pts_per_play_2020_Col2 = Rank_Pts_per_play_2020) %>%
    mutate(Rank_Def_Pts_per_play_2020 = dense_rank(Def_Pts_per_play_2020)) %>%
    mutate(Rank_Def_Pts_per_play_2020_Col2 = Rank_Def_Pts_per_play_2020) %>%
    mutate(Rank_Yd_Diff_2020 = dense_rank(desc(Yd_Diff_2020))) %>%
    mutate(Rank_Yd_Diff_2020_Col2 = dense_rank(desc(Yd_Diff_2020))) %>%
    mutate(Rank_SOS_Adj_MOV_2020 = dense_rank(desc(SOS_Adj_MOV_2020))) %>%
    mutate(Rank_SOS_Adj_MOV_2020_Col2 = dense_rank(desc(SOS_Adj_MOV_2020))) %>%
    mutate(Rank_SOR_Adj_MOV_2020 = dense_rank(desc(SOR_Adj_MOV_2020))) %>%
    mutate(Rank_SOR_Adj_MOV_2020_Col2 = Rank_SOR_Adj_MOV_2020)
...
## setting strings for table title, spreadsheet pathway, unintelligible charts
output_dir <- here("RVoA", "Outputs")
## output_dir <- "~/Documents/Personal/VoA/RVoA/Outputs/"
VoAString <- "VoA.xlsx"
week_text <- "Week"
title_string2 <- "Vortex of Accuracy Top 25"
top25_png <- "VoATop25.png"
fulltable_png <- "VoAFullTable.png"
# new string for full 130 teams table
title_string3 <- "Vortex of Accuracy"
AAC_text <- "AAC"
ACC_text <- "ACC"
Big12_text <- "Big12"
Big10_text <- "Big10"
...
VoATop25Table <- Final_gt_Top25 %>%
  ...
  gt_theme_espn() %>%
  tab_header(
    title = paste(year, week_text, week, title_string2), # ...with this title
...
VoA_Full_Table <- Final_gt_table %>%
  ...
  gt_theme_538() %>%
  tab_header(
    title = paste(year, week_text, week, title_string3), # ...with this title
    subtitle = "Supremely Excellent Yet Salaciously Godlike And Infallibly Magnificent Vortex of Accuracy")  %>%  # and this subtitle
  ##tab_style(style = cell_fill("bisque"),
    ##        locations = cells_body()) %>%  # add fill color to table
  fmt_number( # A column (numeric data)
...
#   tab_header(
#     title = paste(year, week_text, week, title_string2), # Add a title
